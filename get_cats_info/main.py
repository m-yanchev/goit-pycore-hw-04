...
def read_cat_list( path: str ) -> list | None :
    try:
        ...
        return [line.strip() for line in cat_str] # remove leading and trailing whitespace characters
    
def get_cat_dict( cat_str: str ) -> dict | None :

    cat_info = cat_str.split(',')
    if len( cat_info ) != 3:
        print( f"У рядку {cat_str} є зайві дані: { cat_info[3:] }. Вони будуть проігноровані." )
    id = cat_info[0].strip()
    name = cat_info[1].strip().capitalize()
    age = cat_info[2].strip()

    if not is_id(id):
        print(f"Некоректний формат id: {id}. Він повинен бути шістнадцятковим числом.")
        return None
    elif not is_name(name):
        print(f"Некоректний формат імені: {name}. Ім'я не може бути порожнім.")
        return None
    elif not is_age(age):
        print(f"Некоректний формат віку: {age}. Вік повинен бути невід'ємним цілим числом.")
        return None
    else:
